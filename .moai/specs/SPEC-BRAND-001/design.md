...
**⚠️ 새로운 불일치 발견 — 사이즈 범위 표기 충돌 (미해소, 후속 필요).** 이 원천 텍스트는 사이즈 범위를 **"240–330mm"**로 명시한다. 이는 `/bespoke`가 표시하는 **"285mm부터 330mm까지"**(REQ-BRAND-017 명문 요구사항, `spec.md` L185 / `acceptance.md` AC-BRAND-018 — `bespoke-story-pages.test.tsx`의 `/285mm/` 리터럴 단언까지 고정됨)와 모순된다.

이 SPEC-BODY 잠금(REQ-BRAND-017/AC-BRAND-018)은 design/run phase의 편집 권한 밖이다 — `/bespoke`의 표시값을 **바꾸지 않았다**. 이 불일치는 REQ-BRAND-017 자체를 재개봉해야 해소되는 SPEC-body 수준 충돌이며, manager-spec 소유의 후속 작업으로 남긴다(§8.3 갱신 참조). `/story`(신규 CONFIRMED 원천)와 `/bespoke`(SPEC-locked 값)가 서로 다른 사이즈 범위를 표시하는 상태가 이 SPEC 종료 시점에도 남는다 — 사용자 표시 불일치이지 코드 결함은 아니다.
...
| 4 | `/story` 카피 본문 | §2.5 | 원천 축자 전사가 아니라 신규 작성 | ✅ **CONFIRMED** — 축자 원문으로 교체. 단 **신규 불일치 발견**: `/story`의 "240–330mm"가 `/bespoke`의 REQ-BRAND-017-locked "285-330mm"와 충돌 — SPEC-body 재개봉 필요한 별도 후속 항목으로 승격(아래 §8.3-보정 참조) |
...
**신규 후속 항목 (1건, SPEC-body 재개봉 필요)**: `/story`(240–330mm, CONFIRMED 라이브 원천)와 `/bespoke`(285-330mm, REQ-BRAND-017/AC-BRAND-018로 SPEC-body에 잠금)의 사이즈 범위 표기 불일치. design/run phase의 편집 권한 밖이므로 이 보정 사이클에서는 `/bespoke`를 변경하지 않았다 — manager-spec이 REQ-BRAND-017을 재검토해 어느 값이 맞는지 결정하고, `acceptance.md`의 AC-BRAND-018과 `bespoke-story-pages.test.tsx`의 `/285mm/` 단언을 함께 갱신해야 한다.

**권고 후속 백로그 카드**: `moai todo add "SPEC-BRAND-001 후속: PROVISIONAL 2건(이미지 경로/slug) 재확인 + /story-/bespoke 사이즈 범위 불일치(240-330mm vs 285-330mm) REQ-BRAND-017 재검토"`.
