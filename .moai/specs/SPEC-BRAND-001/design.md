...
### §3.1 품절 상품의 사이즈 그리드 표현 — **확정: 전부-비활성 그리드(필수) + 태그 오버레이(부가)**
...
**확정 결정**: 품절 상품(`stock === 0`)은 사이즈 그리드를 **반드시 렌더링**한다 — 5개 사이즈 버튼 전부 `disabled` 상태로 노출한다(REQ-BRAND-022/AC-BRAND-023). 여기에 더해 UI Kit의 품절 태그 오버레이 패턴(불투명도 62% + "품절" 태그)을 **상품 이미지/카드 위에 부가 레이어로** 적용한다 — 이 오버레이는 사이즈 그리드를 대체하는 것이 아니라 이미지 위에 얹는 별도의 시각 신호다. 재고 있는 상품은 사이즈 그리드를 전부 활성 상태로 렌더링한다. 이렇게 하면 "전부 비활성" 그리드와 "일부만 비활성"인 목업 원본의 시각 언어가 섞이지 않는다 — 개별 사이즈에 서로 다른 상태를 부여하지 않기 때문이다.

**제약 준수**: 개별 사이즈에 서로 다른 상태를 부여하지 않는다(REQ-BRAND-023) — 사이즈 그리드는 재고 여부에 따라 5개 전부 동일 상태로 렌더링된다: 재고 있음 = 전부 활성, `stock === 0` = 전부 `disabled`(그리드 자체는 항상 렌더링됨, REQ-BRAND-022/AC-BRAND-023).
...
- [x] 카테고리 3개/4개 확정 (§3.5) — ✅ **CONFIRMED**: 라이브 재조회 결과 3개(더비/로퍼/부츠)로 확정, 몽크 카테고리 폐기
...
- 시드 실행 후 `Category.slug` 3개(`derby`/`loafer`/`boots`) 실제 DB 값 확인
