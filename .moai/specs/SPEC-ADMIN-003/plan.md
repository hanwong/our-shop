...
spec.md §1 "소비하는 계약 — 테스트 파일 봉투" 표의 네 파일을 하나의 단위로 갱신한다(REQ-ADMIN-054·055). M1이 import 지정자를 이미 갱신했으므로 여기서 남는 것은 **문자열 리터럴** — `typecheck`도 `npm test` 통과도 잡아 주지 못하는 것들이다.
...
- **`tests/` 갱신을 "터지면 그때 고친다"로 미루기.** 문자열 리터럴·문서 주석·구조 가드 경로는 `typecheck`도 녹색 `npm test`도 잡아 주지 않는다. 계획 없이 최종 게이트에서 터지는 것이 iteration 1이 지적한 실패 모드다.
