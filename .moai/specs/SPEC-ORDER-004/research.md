...
## §2. 다시 써야 할 기존 테스트 — 2026-09-05 M1 재스캔 스냅샷(6개 파일 12건, 최종 범위 아님)
...
이 수치가 AC-ORDER-072(전체 스위트 기준선 대조)의 기준이며, 게스트 경로 무회귀는 AC-ORDER-071이 별도로 다룬다.

재작성 대상의 실제 범위는 고정된 파일 수로 단정하지 않는다. M1(스키마), M3(리포지토리), M4(서비스)처럼 공유 시그니처를 바꾸는 마일스톤이 착지할 때마다 `npx tsc --noEmit`와 `npx vitest run --reporter=dot`을 전체 코퍼스에 대해 실행하고, 새로 실패하는 파일을 전부 M6 재작성 대상으로 나열한다. 시그니처를 바꾸는 마일스톤(M1, M3, M4, M5) 전부가 착지하기 전에는 고정된 파일/항목 수를 단정하지 않는다 — M6의 실제 범위는 M5 착지 이후에야 완전히 알 수 있다(참고: 이 SPEC에서 최종적으로 확인된 실제 파일 목록은 `progress.md` §E.2의 최종 정정 기록을 보라 — 총 14개 파일). 이 재작성은 새 테스트 파일을 추가하는 것이 아니라 기존 단언을 옮기거나 갱신하는 것이므로, 최종 수치가 1526보다 **작으면** 어딘가에서 커버리지를 잃은 것이고, 그 자체가 실패 신호다.
