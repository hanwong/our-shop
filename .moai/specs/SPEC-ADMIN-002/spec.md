...
**이 표는 파일 단위로 센다.** 완료된 SPEC이 소유한 파일 중 이 SPEC이 변경하는 것 **전부(6개 파일)**이며, `plan.md` §3 EXTEND 표와 같은 항목·같은 단위다. REQ-ADMIN-041의 예외 조항과 AC-ADMIN-041의 검증 대상이 모두 이 표 하나를 가리킨다 — 두 곳이 서로 다른 경계를 말하지 않도록 단일 출처로 둔다.
...
| `tests/integration/catalog/search.test.ts` | `SPEC-CATALOG-002` | REQ-ADMIN-034/035가 저장소의 `where`를 좁히고 `findProductById`를 `findFirst`로 바꾸는 데 따른 **기계적 5곳 갱신**: (1) `:45`에 `findFirst` 모킹 선언 추가 + `:53`의 prisma 클라이언트 배선, (2) `:213` 부근 `beforeEach`에 `findFirst` 리셋 추가, (3) `:405`의 `findUnique.mockResolvedValue`를 `findFirst`로 치환, (4) `:429`의 호출 단언을 `findFirst.mock.calls`로 바꾸고 기대 `where`를 `{ id: "p-shirt", isActive: true }`로, (5) `lastWhere()` 기댓값 3곳(`:309~312`·`:457`에 `isActive: true` 추가, `:463`은 `{}` → `{ isActive: true }`). 모킹·기댓값 갱신일 뿐 로직 변경이 아니며, `SPEC-CATALOG-002`의 spec.md·acceptance.md 본문은 건드리지 않는다 |
